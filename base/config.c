...
int read_main_config_file(char *main_config_file) {
	...
	while(1) {
		...
		if((temp_ptr = my_strtok(input, "=")) == NULL) {
			(void)asprintf(&error_message, "NULL variable");
			error = TRUE;
			break;
			}
		if((variable = (char *)strdup(temp_ptr)) == NULL) {
			(void)asprintf(&error_message, "malloc() error");
			error = TRUE;
			break;
			}
		...
		if((temp_ptr = my_strtok(NULL, "\n")) == NULL) {
			(void)asprintf(&error_message, "NULL value");
			error = TRUE;
			break;
			}
		...
		if(!strcmp(variable, "resource_file")) {
			...
			if(sleep_time <= 0.0) {
				(void)asprintf(&error_message, "Illegal value for sleep_time");
				error = TRUE;
				break;
				}
			}
